...
export default function DatePicker({
  ...
  onChange,
  required = false,
  minAge = 13, // Default age constraint
  errorText,
}) {
  ...
  return (
    <div className="w-full relative" ref={containerRef}>
      {label && (
        <label className="text-[11px] font-bold text-slate-700 mb-1 ml-1 block lowercase tracking-wider">
      ...
      <div 
        ...
        className={`w-full border-2 rounded-xl px-4 h-11 text-xs cursor-pointer flex justify-between items-center transition-all ${isOpen ? 'border-black bg-white' : 'border-slate-100 bg-slate-50 hover:border-black'}`}
      >
        <span className={value ? "text-slate-900 font-black" : "text-slate-400 uppercase tracking-widest font-black"}>
          {value ? new Date(value).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : "DD/MM/YYYY"}
        </span>
        <svg className={`w-3.5 h-3.5 text-slate-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 9l-7 7-7-7" /></svg>
      </div>
      ...
      {errorText && (
        <p className="text-[10px] text-red-500 font-bold ml-1 mt-1">{errorText}</p>
      )}
    </div>
  );
}